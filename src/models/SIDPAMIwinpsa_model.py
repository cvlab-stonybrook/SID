...
class SIDPAMIWInpsaModel(DistangleModel):
    def name(self):
        return 'Shadow Image Decomposition model PAMI19 weighted boundary loss with inpainting networki standalone'
...
#        parser.add_argument('--lambda_smooth',type=float,default=100)
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['G_param','alpha','reconstruction','bd','smooth','inpt','reconstruction2']
        ...
        self.netM = networks.define_G(6, 3, opt.ngf, 'unet_256', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netI = networks.define_G(6, 3, opt.ngf, 'unet_256', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        if self.isTrain:
    ...
    def forward(self):
        ...
        inputM = torch.cat([self.input_img,self.litgt],1)
        self.alpha_pred = self.netM(inputM)
        self.alpha_pred = (self.alpha_pred +1) /2        
        self.final = (self.input_img/2+0.5)*(1-self.alpha_pred) + self.litgt*(self.alpha_pred)
        self.final = self.final*2-1

        inputI = torch.cat([self.input_img,self.final.detach()],1)
    ...
    def backward(self):
        ...
        self.loss_G_param = criterion(self.out,self.shadowfree_img) * self.opt.lambda_res 
        ...
        self.loss = self.loss_reconstruction + self.loss_G_param + self.loss_bd  + self.loss_reconstruction2
    ...
    def get_prediction(self,input):
        ...
        self.final = self.final*2-1
        inputI = torch.cat([self.input_img,self.final],1)
        self.residual = self.netI(inputI)
        self.final_final = self.final+self.residual
