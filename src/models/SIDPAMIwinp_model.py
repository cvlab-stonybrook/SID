...
class SIDPAMIWInpModel(DistangleModel):
    def name(self):
        return 'Shadow Image Decomposition model PAMI19 weighted boundary loss with inpainting network'
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_residual',type=float,default=100)
#        parser.add_argument('--lambda_smooth',type=float,default=100)
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['reconstruction_param','alpha','reconstruction','bd','smooth','inpt','reconstruction2']
        # specify the images you want to save/display. The program will call base_model.get_current_visuals
        self.visual_names = ['input_img', 'out','final','final_final','residual']
        # specify the models you want to save to the disk. The program will call base_model.save_networks and base_model.load_networks
        self.model_names = ['G','M','I']
        ...
        self.netM = networks.define_G(7, 3, opt.ngf, 'unet_256', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netI = networks.define_G(7, 3, opt.ngf, 'unet_256', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
        self.netG.to(self.device)
        self.netM.to(self.device)
        self.netI.to(self.device)
        if self.isTrain:
            ...
            self.optimizer_M = torch.optim.Adam(self.netM.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, 0.999),weight_decay=1e-5)
            self.optimizer_I = torch.optim.Adam(self.netI.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, 0.999),weight_decay=1e-5)
    ...
    def forward(self):
        ...
        inputI = torch.cat([self.input_img,self.final.detach(),self.shadow_mask],1)
        self.residual = self.netI(inputI)
        self.final_final = self.final+self.residual
        #GAN input:
...
#        self.loss_G_param = criterion(self.shadow_param_pred, self.shadow_param) * self.opt.lambda_param 
        self.loss_reconstruction = criterion(self.final,self.shadowfree_img) * self.opt.lambda_res 
        self.loss_reconstruction_param = criterion(self.out,self.shadowfree_img) * self.opt.lambda_res 

        self.loss_bd = criterion(self.final[self.penumbra>0],self.shadowfree_img[self.penumbra>0]) * self.opt.lambda_bd

        self.loss_reconstruction2 = criterion(self.final_final,self.shadowfree_img) * self.opt.lambda_res
        self.loss = self.loss_reconstruction + self.loss_reconstruction_param + self.loss_bd  + self.loss_reconstruction2
        if self.opt.lambda_smooth >0:
            self.loss_smooth = smooth_loss(self.alpha_pred) * self.opt.lambda_smooth
            self.loss += self.loss_smooth
        self.loss.backward()
    ...
    def get_prediction(self,input):
        ...
        self.final = (self.input_img/2+0.5)*(1-self.alpha_pred) + self.lit*self.alpha_pred
        self.final = self.final*2-1
        inputI = torch.cat([self.input_img,self.final,self.shadow_mask],1)
        self.residual = self.netI(inputI)
        self.final_final = self.final+self.residual

        RES = dict()
        RES['phase2']= util.tensor2im(self.final,scale =0)
        RES['final']= util.tensor2im(self.final_final,scale =0)
        RES['phase1'] = util.tensor2im(self.out,scale =0)
        RES['lit'] = util.tensor2im(self.lit,scale =0)
        #RES['param']= self.shadow_param_pred.detach().cpu() 
        RES['matte'] = util.tensor2im(self.alpha_pred.detach().cpu(),scale =0)
        RES['residual'] = util.tensor2im(self.residual.detach().cpu(),scale =0)
...
#        '''
        ...
        residual = F.upsample(self.residual,(w,h),mode='bilinear',align_corners=True)
        final  = input_img_ori * (1-alpha_pred) + lit_ori*(alpha_pred)
        final = final*2 -1 + residual 
